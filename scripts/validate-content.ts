...
const checkLinkTarget = (
  ...
  if (!resolved.startsWith(CONTENT_ROOT)) {
    addError(file, `Relative link escapes content root: '${url}'.`);
    return;
  }

  if (!existsSync(resolved)) {
    addError(file, `Missing asset or file for link '${url}'.`);
  }
};

const writingFiles = await fg('content/writing/*.md');
const courseFiles = await fg('content/courses/**/*.md');
const courseDirs = await fg('content/courses/*', { onlyDirectories: true });
...
for (const file of courseFiles) {
  ...
  if (lessons.has(slug)) {
    addError(file, `Duplicate lesson slug '${slug}' in course '${courseDir}'.`);
  }
...
for (const file of writingFiles) {
  ...
  checkFrontmatter(file, data, WRITING_REQUIRED, true);
}
...
for (const file of courseFiles) {
  ...
  checkFrontmatter(file, data, COURSE_REQUIRED, false);
