...
async function getLastCommitISO(filePath) {
  try {
    ...
    console.error(`Failed to read git history for ${filePath}:`, err?.message || err);
    return null;
  }
}
...
async function getFirstCommitISO(filePath) {
  try {
    ...
    console.error(`Failed to read first commit for ${filePath}:`, err?.message || err);
    return null;
  }
}
