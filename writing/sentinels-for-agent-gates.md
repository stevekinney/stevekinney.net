...
There's a second, sneakier rule here that's worth calling out: writing the marker and running `gh pr create` have to be _separate_ tool calls. The harness sees a whole shell invocation as a single tool call, and the hook fires once, before any of it runs. So if you cram "write the marker && create the PR" into one command, the hook looks _before_ the `touch` half has executed, finds no marker, and—wait, no, that would block you. The actual failure is subtler and worse: the hook can't see _into_ your compound command to know a `gh pr create` is hiding in there after the `&&`, so a determined one-liner can smuggle the create past a gate that only pattern-matches the command it was shown. Either way, the lesson is the same: the proof has to land in its own call, before the action it's guarding runs in another.
