...
You have deployed a Lambda function, invoked it, and it works. But if you invoke it after a period of inactivity, the first response is noticeably slower than the ones that follow. That first slow response is a **cold start**, and it is the performance characteristic of Lambda that matters most to frontend engineers building API backends.

A cold start adds latency to the first request after an idle period. For an API endpoint that your frontend calls on page load, that latency is the difference between a snappy experience and a visible loading spinner. You cannot eliminate cold starts entirely, but you can make them fast enough that users do not notice.
...
This is the highest-impact change you can make. Remove unused dependencies, use tree-shaking, and audit your `package.json` regularly.
...
A good target for a typical API handler is under 5 MB zipped. If your zip is over 10 MB, you probably have dependencies you do not need.
...
This keeps the init phase fast for the common case. The uncommon path pays the import cost only when it is actually used.
...
This keeps 5 execution environments warm at all times. You pay for them whether they handle requests or not — it is essentially the cost of running a small server continuously.
...
> [!TIP]
> Before reaching for provisioned concurrency, measure your actual cold start latency. If your function initializes in 200 milliseconds and your API's p95 latency requirement is under 500 milliseconds, you are already fine. Optimize based on measurement, not anxiety.
...
The `Init Duration` field only appears on cold start invocations. If you do not see it, the invocation was a warm start. Compare the `Duration` (your handler execution time) to the `Init Duration` (the cold start overhead) to understand where your latency comes from.

## What is Next

You now understand all the core Lambda concepts: writing handlers, configuring execution roles, deploying and testing, setting environment variables, and managing cold start performance. In the next module, you will put an HTTP layer in front of your function with API Gateway, so your frontend can call it with a `fetch` request.
