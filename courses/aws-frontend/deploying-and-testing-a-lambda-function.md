...
The zip file must contain your handler file at the root level — not nested inside a `dist/` folder. That is why you `cd` into `dist/` before zipping: Lambda looks for the handler at the path you specify (e.g., `handler.handler`), and if your file is at `dist/handler.js` inside the zip, Lambda will not find it.

> [!WARNING]
> If your function uses `node_modules` dependencies (beyond what the Lambda runtime provides), you need to include them in the zip. Copy `node_modules` into the `dist/` directory before zipping. For this lesson, the handler has no runtime dependencies — `@types/aws-lambda` is a dev dependency that is only used during compilation.
...
- **`--handler`**: The path to your handler function in the format `<filename>.<export>`. Since your compiled file is `handler.js` and the exported function is `handler`, this is `handler.handler`.
- **`--zip-file`**: The deployment package. The `fileb://` prefix tells the CLI to read the file as binary data.

The response confirms the function was created:
...
> [!TIP]
> The `--cli-binary-format raw-in-base64-out` flag is required when passing `--payload` with AWS CLI v2. Without it, the CLI tries to base64-encode your payload, which is not what you want for JSON test events.

## Using Test Event Files

Typing JSON payloads on the command line gets unwieldy fast. Instead, save your test event to a file:
...
You will not create a new function every time you change your code. After the initial `create-function`, use `update-function-code` to deploy changes:
...
We will cover CloudWatch in depth in Module 12. For now, this command is your debugging tool.

> [!TIP]
> You can also include `--log-type Tail` on your `aws lambda invoke` command. This returns the last 4 KB of log output as a base64-encoded string in the response, which you can decode with `base64 --decode`. It is quicker than querying CloudWatch for simple debugging.

## The Full Deployment Script

Here is the complete build-and-deploy workflow as a single script. You will run some version of this every time you update your function:
...
This is the manual version. Later in the course, you will automate this in a GitHub Actions workflow, just like you automated S3 deployments in Module 6.

## Common Mistakes

**Wrong handler path.** If you get "Runtime.HandlerNotFound" when invoking, your `--handler` value does not match the file and export name inside the zip. Double-check that `handler.js` is at the root of the zip (not in a subdirectory) and that the exported function is named `handler`.
...
Your function is deployed and you can invoke it directly. But direct invocation is not how a frontend calls an API — you need an HTTP endpoint. Before we get to API Gateway in Module 8, there are two more Lambda topics to cover: environment variables (for configuration that should not be hardcoded) and cold starts (the performance characteristic that matters most for API latency).
