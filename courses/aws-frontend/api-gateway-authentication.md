...
Your API is deployed, CORS is configured, and your frontend can call it. But right now, anyone with the URL can call any endpoint. There is nothing stopping a random script from hammering `POST /items` with garbage data. You need authentication — a way to verify that the person making the request is who they claim to be and that they are allowed to do what they are asking.
...
Here is how to create a JWT authorizer that validates tokens from an Amazon Cognito user pool:
...
- **`--jwt-configuration Issuer`** — the issuer URL. For Cognito, this is `https://cognito-idp.{region}.amazonaws.com/{userPoolId}`. For Auth0, it is `https://{your-domain}.auth0.com/`. The issuer must match the `iss` claim in the JWT.
...
API Gateway fetches the provider's JWKS (the public keys used to sign tokens) from the issuer's `/.well-known/openid-configuration` endpoint and uses them to verify token signatures. You do not need to manage keys yourself.

Here is the same command for an Auth0-issued token:
...
> [!TIP]
> The issuer URL for Auth0 must include the trailing slash. Cognito issuer URLs do not include a trailing slash. This is a common source of 401 errors — the issuer in the token must exactly match the issuer you configure on the authorizer.

### Attaching the Authorizer to a Route

Creating an authorizer does not protect any routes by itself. You need to attach it to specific routes using `update-route`:
...
The `--authorization-scopes` parameter is optional. If specified, API Gateway checks that the token's `scope` claim contains at least one of the listed scopes. This lets you create fine-grained permissions: a `GET /items` route might require `items:read`, while `POST /items` requires `items:write`.
...
**Lambda authorizers** run a Lambda function to decide whether a request is allowed. They are more flexible than JWT authorizers — you can validate tokens from non-standard providers, check custom headers, look up API keys in a database, or implement any authentication logic you need.
...
> [!WARNING]
> Lambda authorizer invocations count as additional Lambda invocations and incur their own costs. The `--authorizer-result-ttl-in-seconds` parameter caches the authorization result so the authorizer is not called on every request. Set this to a reasonable value (300 seconds is five minutes) to reduce both latency and cost.
...
Start with JWT authorizers. Move to Lambda authorizers when you need logic that JWT validation cannot express — checking a database, validating a custom token format, or implementing multi-tenant authorization.
...
You now have the complete API Gateway picture: HTTP APIs, routes, integrations, CORS, stages, custom domains, and authentication. In the exercise that follows, you will put all of these pieces together and build a working API that your frontend can call.
