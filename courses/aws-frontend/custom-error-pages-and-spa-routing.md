...
If you have deployed a single-page application to Vercel or Netlify, you know the drill: you add a `rewrites` rule or a `_redirects` file so that every path serves `index.html`, and your client-side router (React Router, Vue Router, whatever) handles the URL. Without that rule, refreshing the page on `/dashboard/settings` returns a 404 because no file exists at that path — the server does not know that your JavaScript handles routing.

On AWS, the same problem exists, and the fix is CloudFront's **custom error responses**.

## The Problem

Here is what happens when a user navigates to `/dashboard/settings` on your CloudFront distribution:
...
5. S3 returns a **403 Forbidden** error (because with Origin Access Control, S3 returns 403 instead of 404 for missing objects).
...
- **403**: S3 returns this for missing objects when using OAC. It also returns 403 if the bucket policy does not allow the request.
...
The path to the file CloudFront should serve instead of the error. This must start with `/` and reference a file that exists in your S3 bucket. For a SPA, that is `/index.html`.

### ResponseCode

The HTTP status code CloudFront returns to the browser along with the custom response page. This is the critical field. You have two choices:
...
> [!WARNING]
> Do not set `ErrorCachingMinTTL` to `0`. CloudFront requires error responses to be cached for at least some duration. A value of `10` is a reasonable balance — short enough that errors do not persist long, but long enough that CloudFront is not hammering your origin on every request for a missing file.
...
The `200` status confirms the custom error response is working. The `x-cache: Miss from cloudfront` header tells you this was not cached yet. A subsequent request to the same path will show `Hit from cloudfront`.
...
This is the same behavior as Vercel's rewrite rules and Netlify's `_redirects`. It is a trade-off: you lose HTTP-level 404 semantics in exchange for working client-side routing. For most single-page applications, this is the correct trade-off. Your frontend router knows which routes are valid and which are not — the CDN does not.

If you need proper 404 status codes for SEO purposes (e.g., a marketing site with some SPA sections), you would handle this with a CloudFront Function or Lambda@Edge that inspects the URL and returns 404 for genuinely invalid paths. That is covered in Module 9.

## Multi-Page Sites vs. SPAs

If you are deploying a traditional multi-page static site (not a SPA), you probably do not want this configuration. A 404 for a missing page should be an actual 404, not a redirect to `index.html`. In that case, you would configure a custom error response that returns a dedicated error page:
...
Your SPA routes work. But your site is still served on a `*.cloudfront.net` domain, and there is no custom SSL certificate. In the next lesson, you will attach the ACM certificate you provisioned in Module 3 to your CloudFront distribution, enabling HTTPS on your custom domain.
