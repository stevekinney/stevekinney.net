...
You know what Lambda is and how the execution model works. Now you need to write the code that Lambda actually runs. A Lambda **handler** is just an exported async function with a specific signature — it receives an event, does some work, and returns a response. If you have ever written an API route in Next.js or a serverless function in Vercel, the shape is nearly identical.
...
The key dependency here is `@types/aws-lambda` — this package provides TypeScript type definitions for every Lambda event source. You will not install the AWS SDK as a dependency because Lambda provides it in the execution environment already. You only need the types.
...
Every Lambda handler follows the same pattern: it is an exported async function that receives an `event` and returns a response. The type of the event depends on what triggers the function. Since you are building an API backend that will sit behind API Gateway (covered in Module 8), you will use the `APIGatewayProxyHandlerV2` type.

Here is the minimal handler:
...
The "V2" in the name refers to API Gateway's HTTP API (the newer, cheaper, faster version). There is also `APIGatewayProxyHandler` (without V2) for the older REST API. This course uses HTTP APIs, so you will use the V2 types throughout.

### The Event Object

The `event` object contains everything about the incoming HTTP request. Here are the fields you will use most often:
...
The response must include a `statusCode` and a `body`. The `body` must be a string — if you are returning JSON, you need to `JSON.stringify` it yourself. The `headers` object is optional but you should always set `Content-Type`.
...
Here is a more realistic handler that demonstrates parsing query parameters, handling different HTTP methods, and returning appropriate error responses:
...
- Checks the HTTP method and returns a 405 if it is not a GET request
...
You will not use the context object in every handler, but it is good to know it is there.
...
TypeScript compiles `src/handler.ts` to `dist/handler.js`. The compiled JavaScript file is what Lambda actually runs. In the next lesson, you will package this output into a zip file and deploy it.

## Common Mistakes

**Returning an object instead of a stringified body.** Lambda expects `body` to be a string. If you pass an object, the response will be mangled.
...
**Using callbacks instead of async/await.** Lambda supports both patterns, but the callback pattern (`callback(null, response)`) is a holdover from the Node.js 6 era. Use async handlers. They are cleaner, they work with try/catch, and they are what the types expect.
...
You have a compiled TypeScript handler ready to go. Before you can deploy it, you need an IAM execution role — the role that Lambda assumes when it runs your function. That role determines what AWS services your function can access. You will create one in the next lesson, building on the IAM concepts from [The IAM Mental Model](iam-mental-model.md).
