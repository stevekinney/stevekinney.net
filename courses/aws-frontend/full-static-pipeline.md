...
Four services, four clearly defined responsibilities. No servers. No containers. No load balancers.

## The Order of Operations

The order matters. You cannot attach a certificate to a distribution that does not exist. You cannot create an alias record for a distribution that has no alternate domain name. Here is the correct sequence and why each step depends on the previous one.
...
At this stage, the bucket can be public or private. It does not matter yet because you will lock it down with Origin Access Control once CloudFront is in place.
...
> [!WARNING]
> The certificate must be in `us-east-1`. CloudFront is a global service, but it only reads certificates from `us-east-1`. You covered this requirement in [Certificate Renewal and us-east-1](certificate-renewal-and-us-east-1.md). If your certificate is in any other region, CloudFront will not see it.
...
> [!TIP]
> If your domain is registered outside Route 53, you need to update the nameservers at your registrar to point to Route 53's nameservers. This was covered in [Registering and Transferring Domains](registering-and-transferring-domains.md). DNS propagation can take up to 48 hours when changing nameservers, though it is often faster.
...
If you try to create Route 53 records before the distribution exists, the alias target does not resolve. If you try to attach a certificate before it is issued, CloudFront rejects the configuration. If you try to configure OAC before the distribution exists, there is nothing to attach it to. The order is not arbitrary: it is dictated by the dependencies between services.
...
You built a scoped deploy bot with exactly these S3 and CloudFront permissions in the [IAM Policy for a Deploy Bot exercise](iam-policy-exercise.md). That same policy is what you will use in CI/CD pipelines.

## What Each Service Is Doing at Runtime

Once the pipeline is deployed, here is what happens when a user visits `https://example.com/dashboard`:
...
4. **Cache miss (first visit)**: CloudFront sends a signed request to S3 (using OAC's SigV4 credentials). S3 does not have a file at `/dashboard`, so it returns a 403.
...
This is the foundation. In the next two lessons, you will wrap these commands in a deploy script and then automate them with GitHub Actions so deployments happen on every push to `main`.

> [!TIP]
> If you want to test the pipeline before automating it, run the two commands above manually after each build. That is exactly what the automated pipeline does: it just removes you from the loop.
