...
What you've built here—correlation IDs plus structured logging plus Insights queries—is the foundation of request tracing. AWS offers more sophisticated tracing through **AWS X-Ray**, which automatically instruments SDK calls and produces visual service maps. X-Ray is the underlying infrastructure; **CloudWatch Application Signals** (GA July 2024) is the modern console surface where you view distributed traces, service maps, and SLOs. The open-source path is the **AWS Distro for OpenTelemetry (ADOT)**—AWS's supported OpenTelemetry distribution, which lets you send traces to X-Ray _and_ to third-party OTLP-compatible backends (Datadog, Honeycomb, Grafana Tempo). If you end up in a job description asking for "OpenTelemetry on AWS," ADOT is what they mean. All three are beyond the scope of this course, but the structured logging patterns you've learned here work alongside them—correlation IDs and structured logs are the minimum viable observability for any production application.
