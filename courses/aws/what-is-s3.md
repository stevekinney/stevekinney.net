...
If you want AWS's canonical version of the service definition while you read, the [Amazon S3 User Guide](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Welcome.html) is the official reference.
...
If you understand S3 correctly, the rest of the static-hosting stack stops feeling magical. CloudFront becomes "cache in front of a bucket," Route 53 becomes "DNS that points at the cache," and deployment becomes "replace objects predictably." If you misunderstand S3, every later service feels arbitrary.

## Builds On

This lesson builds on the deployment model you already know from Vercel and Netlify: frontend assets are static files generated by a build step and uploaded somewhere durable. The new piece is that AWS exposes the storage layer directly instead of hiding it behind a platform abstraction.

![Diagram showing an S3 bucket containing flat object keys with virtual prefixes and how S3 sits under CloudFront, ACM, and Route 53 in the frontend stack.](assets/s3-object-storage-mental-model.svg)
...
> [!NOTE] Object storage in plain English
> **Object storage** stores data as whole objects inside a bucket. Each object has three parts: a key, the data itself, and metadata about that data.
>
> **How it's different:** a traditional file system gives you directories, nested paths, and files you can edit in place. Object storage gives you a flat collection of objects addressed by key. If you "move" or "rename" something, you're usually copying the object to a new key and deleting the old one.
>
> **Advantages:** it scales ridiculously well, it's durable, it's cheap for static assets, and it's easy to serve over HTTP.
>
> **Disadvantages:** it's not a great fit for workloads that need low-latency random writes, shared file system behavior, or lots of tiny in-place updates. If you treat S3 like a remote hard drive, it will eventually make you regret your life choices.
...
2. **CloudFront** sits in front of S3 and serves those files from edge locations around the world
3. **ACM** provides the SSL certificate so your site loads over HTTPS on your domain
4. **Route 53** points your custom domain to CloudFront
...
- You can explain the difference between a bucket and an object without using the word "folder."
- You can point at a build artifact like `index.html` or `app.js` and describe it as an S3 object with a key, not a file in a traditional file system.
- You can run `aws s3 ls` or `aws s3api list-buckets` and understand which command is the higher-level convenience wrapper.

## Common Failure Modes

- **Treating S3 like a normal file system:** That mental model leads directly to confusion about "folders," object keys, and bucket policies.
- **Choosing a throwaway bucket name too quickly:** Bucket names are global and effectively permanent in your architecture once other services start depending on them.
- **Assuming upload equals public access:** S3 stores files. It does not automatically turn them into a website or public origin.
