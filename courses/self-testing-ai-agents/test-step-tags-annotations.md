...
In the current course flow, the concrete reps are `tests/rate-book.spec.ts` (created earlier by the [harden-the-flaky-rate-book-test lab](lab-harden-the-flaky-rate-book-test.md), not shipped in the starter) and `tests/smoke.spec.ts`. However you isolate the practice slice for the lab, once the step labels and tags are stable they belong in the normal `npm run test` loop with the rest of the suite.
