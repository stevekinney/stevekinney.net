...
Shelf does exactly this. Look at [`tests/end-to-end/fixtures.ts`](https://github.com/stevekinney/shelf-life/blob/main/tests/end-to-end/fixtures.ts) in the starter — it overrides `page` to forward browser console errors and failed network requests to stderr, so they show up in the `list` reporter, the HTML report, and the failure dossier JSON:
