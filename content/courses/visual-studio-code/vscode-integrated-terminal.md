...
modified: 2025-03-16T16:39:02-06:00
---

The built-in terminal is pretty straight-forward, but let's look at some of the ways that we can take it the Next Level™.
...
> [!TIP] If you often need the same set of terminals (e.g., server, build watcher, test watcher), consider scripting a quick Terminal Task to launch them all in one go. It’s like calling your personal pit crew to get everything running at once.

## Terminal Profiles

One of the integrated terminal’s superpowers is the ability to switch between different shell profiles—bash, zsh, PowerShell, WSL, and more. To configure these profiles, open the Command Palette (`Ctrl+Shift+P` or `Cmd+Shift+P` on macOS) and search for `Terminal: Select Default Profile`. A list of detected shells appears, allowing you to choose your favorite.
...
This feature helps you quickly locate that error message from three minutes ago without scrolling endlessly. Like searching in the editor, you can toggle case sensitivity or match whole words if needed.
