...
- **The `.md` redirect chain lives in `+layout.server.ts` and the page-level loaders.** `/courses/X.md` redirects to `/courses/X`, `/courses/X.md/Y` preserves the lesson segment when `X` is a course, unique legacy lesson slugs redirect to `/courses/<matched-course>/<lesson>`, and `/courses/X/Y.md` redirects to `/courses/X/Y`. `server/content.ts` normalizes `.md` suffixes inside `getCourseEntry` / `getCourseRoute` as a defense in depth.
